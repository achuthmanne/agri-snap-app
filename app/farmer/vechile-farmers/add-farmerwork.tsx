// add-farmerwork.tsx WITH DUPLICATE WARNING SAFETY & ACRES/SAALLU LOGIC
...
export default function AddFarmerWork() {
  ...
  // 🔥 UPDATED CALCULATION LOGIC FOR ACRES VS SAALLU
  useEffect(() => {
  ...
  const cropOptions = [
    ...
    { "en": "Groundnut", "te": "వేరుశనగ" },
    { "en": "Turmeric", "te": "పసుపు" },
    { "en": "Tobacco", "te": "పొగాకు" },
    { "en": "Red Gram (Kandi)", "te": "కంది" },
    { "en": "Green Gram (Pesara)", "te": "పెసర" },
    { "en": "Black Gram (Minumu)", "te": "మినుము" },
    { "en": "Bengal Gram (Senaga)", "te": "శనగ" },
    { "en": "Soya Bean", "te": "సోయాబీన్" },
    { "en": "Sunflower", "te": "సూర్యకాంతి" },
    { "en": "Oil Palm", "te": "ఆయిల్ పామ్" },
    { "en": "Banana", "te": "అరటి" },
    { "en": "Mango", "te": "మామిడి" },
    { "en": "Tomato", "te": "టొమాటో" },
  ...
  useSpeechRecognitionEvent("end", () => {
    if (isMounted.current) { setIsListening(false); setVoiceTarget(null); }
  });
  ...
  const handleSave = async (bypassDuplicate = false) => {
    ...
    try {
      ...
      if (isMounted.current) {
        setSaving(false);
        setErrorMsg(language === "te" ? "నెట్వర్క్ సమస్య, మళ్లీ ప్రయత్నించండి." : "Network error, try again.");
        setErrorModal(true);
      }
    }
  };
  ...
  return (
    <SafeAreaView style={styles.safe}>
      ...
      >
        ...
        <TouchableOpacity
          activeOpacity={0.8}
          style={[styles.inputBox, activeInput === "date" && styles.inputFocused, errors.date && styles.inputError]}
          onPress={() => { setActiveInput("date"); setShowDatePicker(true); if (errors.date) setErrors({ ...errors, date: "" }); }}
        >
        ...
        <TouchableOpacity
          ...
          onPress={() => { setModalType("work"); setActiveInput("work"); if (errors.work) setErrors({ ...errors, work: "" }); }}
        >
              ...
              <View style={styles.inputWrapper}>
                {!acres && activeInput !== "acres" && (
                  <AppText style={{ color: "#9CA3AF", fontFamily: "Mandali" }}>
                    {language === "te" ? "మొత్తం ఎకరాలు నమోదు చేయండి*" : "Enter Total Acres*"}
                  </AppText>
                )}
                <TextInput
                  ref={acresInputRef} value={acres}
                  onChangeText={(txt) => { setAcres(txt); if (errors.acres) setErrors({ ...errors, acres: "" }); }}
                  keyboardType="numeric"
                  style={[styles.input, { display: (acres || activeInput === "acres") ? "flex" : "none" }]}
                  cursorColor="#16A34A" selectionColor="#16A34A40"
                  onFocus={() => setActiveInput("acres")} onBlur={() => setActiveInput(null)}
                />
              </View>
              {acres.length > 0 && (
                <AppText style={styles.unitText}>{language === "te" ? "ఎకరాలు" : "Acres"}</AppText>
              )}
            </TouchableOpacity>
          ...
          <View style={{ marginBottom: errors.rateError ? 0 : 16 }}>
            ...
            <View style={{ flexDirection: "row", gap: 12, marginBottom: 16 }}>
              ...
              <TouchableOpacity activeOpacity={1}
                style={[
                  styles.inputBox, { flex: 1.2, marginBottom: 0 }, 
                  activeInput === "rateAcre" && styles.inputFocused,
                  errors.rateError && !ratePerAcre && styles.inputError
                ]}
                ...
                <View style={styles.inputWrapper}>
                  {!ratePerAcre && activeInput !== "rateAcre" && (
                    <AppText style={{ color: "#9CA3AF", fontSize: 13, fontFamily: "Mandali" }}>
                      {language === "te" ? "ఎకరాకు ధర (₹)" : "Rate/Acre (₹)"}
                    </AppText>
                  )}
                  <TextInput
                    ...
                    onChangeText={(txt) => {
                      setRatePerAcre(txt);
                      // 🔥 Clear Saallu fields if user chooses Acres route
            ...
            <View style={{flexDirection: 'row', alignItems: 'center', marginBottom: 16}}>
              ...
              <AppText style={{marginHorizontal: 10, color: '#6B7280', fontSize: 14, fontWeight: '600'}}>
                {language === 'te' ? "లేదా" : "OR"}
              </AppText>
              <View style={{flex: 1, height: 1, backgroundColor: '#E5E7EB'}}/>
            </View>

            {/* --- OPTION 2: SAALLU BASED (PLOUGHING) --- */}
            <AppText style={styles.label}>
            ...
            <View style={{ flexDirection: "row", gap: 12, marginBottom: 8 }}>
              ...
              <TouchableOpacity activeOpacity={1}
                style={[
                  styles.inputBox, { flex: 1, marginBottom: 0 }, 
                  activeInput === "saallu" && styles.inputFocused,
                  errors.rateError && !saalluCount && styles.inputError
                ]}
              ...
              >
                ...
                <View style={styles.inputWrapper}>
                  {!saalluCount && activeInput !== "saallu" && (
                    <AppText style={{ color: "#9CA3AF", fontSize: 13, fontFamily: "Mandali" }}>
                      {language === "te" ? "సాళ్ల సంఖ్య" : "No. Saallu"}
                    </AppText>
                  )}
                  <TextInput
                    ...
                    onChangeText={(txt) => {
                      setSaalluCount(txt);
                      // 🔥 Clear Acres Rate if user chooses Saallu route
                      setRatePerAcre(""); 
                      if (errors.rateError) setErrors({ ...errors, rateError: "" });
                    }}
                    keyboardType="numeric" cursorColor="#16A34A" selectionColor="#16A34A40"
              ...
              {/* Box 2: Rate Per Saalu */}
              <TouchableOpacity activeOpacity={1}
                style={[
                  styles.inputBox, { flex: 1.2, marginBottom: 0 }, 
                  activeInput === "rateSaalu" && styles.inputFocused,
                  errors.rateError && !ratePerSaalu && styles.inputError
                ]}
              ...
              >
                ...
                <View style={styles.inputWrapper}>
                  ...
                  <AppText style={{ color: "#9CA3AF", fontSize: 13, fontFamily: "Mandali" }}>
  {language === "te" ? "సాలు ధర (₹)" : "Rate/Saalu (₹)"}
</AppText>
            ...
            {/* Error Message */}
            {errors.rateError && (
              <AppText style={[styles.errorText, { marginTop: 4, marginBottom: 10 }]} language={language}>
                {errors.rateError}
              </AppText>
            )}
            ...
            <View style={{ paddingHorizontal: 4, marginTop: 5 }}>
              <View style={{ flexDirection: 'row', alignItems: 'flex-start' }}>
                <Ionicons name="information-circle-outline" size={16} color="#059669" style={{ marginTop: 5 }} />
                <View style={{ marginLeft: 6, flex: 1 }}>
                 <AppText style={{ fontSize: 13, color: "#166534", fontFamily: "Mandali" }}>
  {language === "te" 
    ? `గమనిక: మీ ${acres || 'మొత్తం'} ఎకరాలకు కలిపి సాళ్లు వేయాలి (ఉదా: 2 ఎకరాలు × 2 సార్లు = 4 సాళ్లు).` 
    : `Note: Enter total Saallu for your ${acres || 'total'} acres combined (Ex: 2 Acres × 2 Times = 4 Saallu).`}
</AppText>
                </View>
              </View>
           </View>  
          ...
          <View>
            ...
            <View style={{ marginBottom: errors.duration ? 0 : 16 }}>
              ...
              <View style={{ flexDirection: "row", alignItems: "center", gap: 10 }}>
                <TouchableOpacity
                  activeOpacity={1}
                  style={[styles.inputBox, { flex: 1, marginBottom: 0 }, activeInput === "hrs" && styles.inputFocused, errors.duration && styles.inputError]}
                  onPress={() => { setActiveInput("hrs"); hrsInputRef.current?.focus(); }}
                >
                  <View style={styles.inputWrapper}>
                    {!hrs && activeInput !== "hrs" && (<AppText style={{ color: "#9CA3AF" }}>00</AppText>)}
                    <TextInput
                      ref={hrsInputRef} value={hrs} onChangeText={(txt) => { setHrs(txt); if (errors.duration) setErrors({ ...errors, duration: "" }); }}
                      keyboardType="numeric" maxLength={2} style={[styles.input, { textAlign: 'center', display: (hrs || activeInput === "hrs") ? "flex" : "none" }]}
                      cursorColor="#16A34A" selectionColor="#16A34A40" onFocus={() => setActiveInput("hrs")} onBlur={() => setActiveInput(null)}
                    />
                  </View>
                ...
                <TouchableOpacity
                  activeOpacity={1}
                  style={[styles.inputBox, { flex: 1, marginBottom: 0 }, activeInput === "mins" && styles.inputFocused, errors.duration && styles.inputError]}
                  onPress={() => { setActiveInput("mins"); minsInputRef.current?.focus(); }}
                >
                  <View style={styles.inputWrapper}>
                    {!mins && activeInput !== "mins" && (<AppText style={{ color: "#9CA3AF" }}>00</AppText>)}
                    <TextInput
                      ref={minsInputRef} value={mins}
                      onChangeText={(val) => { if (parseInt(val) < 60 || val === "") setMins(val); if (errors.duration) setErrors({ ...errors, duration: "" }); }}
                      keyboardType="numeric" maxLength={2} style={[styles.input, { textAlign: 'center', display: (mins || activeInput === "mins") ? "flex" : "none" }]}
                      cursorColor="#16A34A" selectionColor="#16A34A40" onFocus={() => setActiveInput("mins")} onBlur={() => setActiveInput(null)}
                    />
                  </View>
            ...
            <View style={{ flexDirection: "row", gap: 12, marginBottom: 16 }}>
              <TouchableOpacity activeOpacity={1}
                style={[styles.inputBox, { flex: 1.5, marginBottom: 0 }, activeInput === "rate" && styles.inputFocused, errors.ratePerHour && styles.inputError]}
                onPress={() => { setActiveInput("rate"); rateInputRef.current?.focus(); }}
              >
                <Ionicons name="cash-outline" size={20} color={ratePerHour || activeInput === "rate" ? "#16A34A" : "#9CA3AF"} />
                <View style={styles.inputWrapper}>
                  {!ratePerHour && activeInput !== "rate" && (
                    <AppText style={{ color: "#9CA3AF", fontFamily: "Mandali" }}>{language === "te" ? "గంటకు ధర (₹)*" : "Rate per Hr (₹)*"}</AppText>
                  )}
                  <TextInput
                    ref={rateInputRef} value={ratePerHour} onChangeText={(txt) => { setRatePerHour(txt); if (errors.ratePerHour) setErrors({ ...errors, ratePerHour: "" }); }}
                    keyboardType="numeric" style={[styles.input, { display: (ratePerHour || activeInput === "rate") ? "flex" : "none" }]}
                    cursorColor="#16A34A" selectionColor="#16A34A40" onFocus={() => setActiveInput("rate")} onBlur={() => setActiveInput(null)}
                  />
                </View>
              </TouchableOpacity>

              <View style={[styles.inputBox, { flex: 1, marginBottom: 0, backgroundColor: "#E5E7EB", borderColor: "#D1D5DB" }]}>
                <View style={[styles.inputWrapper, { marginLeft: 0, alignItems: 'center' }]}>
                  <AppText style={{ fontSize: 11, color: "#6B7280", marginBottom: 2 }}>{language === "te" ? "మొత్తం" : "Total"}</AppText>
                  <AppText style={{ color: "#111827", fontWeight: "bold", fontSize: 16 }}>₹{getCalculationDetails().amount}</AppText>
                </View>
              </View>
            </View>
            {errors.ratePerHour && <AppText style={[styles.errorText, { marginTop: -10 }]} language={language}>{errors.ratePerHour}</AppText>}
          </View>
        )}
        ...
        <View style={{ marginBottom: 16 }}>
          <View style={{ flexDirection: "row", alignItems: "center", gap: 10 }}>
            <View style={{ flex: 1 }}>
              ...
              <TouchableOpacity activeOpacity={1}
                style={[styles.inputBox, { marginBottom: 5 }, activeInput === "payable" && styles.inputFocused, errors.payableAmount && styles.inputError]}
                onPress={() => { setActiveInput("payable"); payableInputRef.current?.focus(); }}
              >
                <TextInput
                  ref={payableInputRef} value={payableAmount}
                  onChangeText={(txt) => { setPayableAmount(txt); if (errors.payableAmount) setErrors({ ...errors, payableAmount: "" }); }}
                  keyboardType="numeric" style={styles.input} cursorColor="#16A34A" selectionColor="#16A34A40"
                  onFocus={() => setActiveInput("payable")} onBlur={() => setActiveInput(null)}
                />
              </TouchableOpacity>
            ...
            <View style={{ flex: 1 }}>
              ...
              <TouchableOpacity activeOpacity={1}
                style={[styles.inputBox, { marginBottom: 5 }, activeInput === "advance" && styles.inputFocused]}
                onPress={() => { setActiveInput("advance"); advanceInputRef.current?.focus(); }}
              >
                <TextInput
                  ref={advanceInputRef} value={advanceAmount} onChangeText={setAdvanceAmount} keyboardType="numeric"
                  style={styles.input} cursorColor="#16A34A" selectionColor="#16A34A40"
                  onFocus={() => setActiveInput("advance")} onBlur={() => setActiveInput(null)}
                />
              </TouchableOpacity>
            </View>
          </View>
          ...
          <View style={{ flexDirection: 'row', alignItems: 'center', marginTop: 6, marginLeft: 4 }}>
            ...
            <AppText style={{ fontSize: 11, color: "#6B7280", marginLeft: 4 }}>
              {language === "te" ? "లెక్కించిన చెల్లింపు మొత్తంలో* తప్పు ఉంటే, మీరు సవరించుకోవచ్చు." : "If the calculated Payable Amount* is incorrect, you can edit it."}
            </AppText>
          </View>
        ...
        <View style={{ marginBottom: 20 }}>
          <AppText style={styles.label}>
            {language === "te" ? "అదనపు సమాచారం (ఐచ్ఛికం)" : "Additional Remarks (Optional)"}
          </AppText>
          <TouchableOpacity activeOpacity={1}
            style={[styles.inputBox, { minHeight: 120, alignItems: "flex-start", paddingVertical: 14, marginBottom: 20 }, activeInput === "notes" && styles.inputFocused]}
            onPress={() => { setActiveInput("notes"); notesInputRef.current?.focus(); }}
          >
            ...
            <View style={[styles.inputWrapper, { marginLeft: 12, flex: 1 }]}>
              ...
              <TextInput
                ref={notesInputRef} value={notes} onChangeText={setNotes} multiline
                placeholder={isListening && voiceTarget === "notes" ? (language === "te" ? "వింటున్నాను..." : "Listening...") : ""} placeholderTextColor="#EF4444"
                style={[styles.input, { lineHeight: 22, minHeight: 80, textAlignVertical: "top", padding: 0, display: notes || activeInput === "notes" ? "flex" : "none" }]}
        ...
        <TouchableOpacity activeOpacity={0.85} style={[styles.saveBtn, {marginTop: 50}]} onPress={() => handleSave(false)} disabled={saving}>
      ...
      {showDatePicker && (
        <DateTimePicker
          ...
          onChange={(event, selectedDate) => {
            setShowDatePicker(false); setActiveInput(null);
            if (selectedDate) {
          ...
          <View style={styles.modalContent}>
            <View style={styles.modalHeader}>
              <AppText style={styles.modalTitleText}>
                {modalType === "crop" ? (language === "te" ? "పంట ఎంచుకోండి" : "Select Crop") : (language === "te" ? "పని ఎంచుకోండి" : "Select Work")}
              </AppText>
            ...
            <View style={styles.searchBar}>
              <TextInput autoFocus value={searchText} onChangeText={(text) => setSearchText(text)} placeholder={language === "te" ? "ఇక్కడ రాయండి..." : "Type here..."} placeholderTextColor="#9CA3AF" cursorColor={'green'} style={{ flex: 1, fontSize: 16, fontFamily: "Mandali", color: "#1F2937", paddingVertical: 8 }} />
              {searchText.trim().length > 0 && (
                <TouchableOpacity onPress={() => { if (modalType === "crop") setCrop(searchText); else setWork(searchText); setModalType(null); setSearchText(""); setActiveInput(null); }} style={{ backgroundColor: "#16A34A", borderRadius: 12, padding: 6, marginLeft: 6 }}>
            ...
            <FlatList
              data={filteredData} keyExtractor={(item, index) => `${item.en}-${index}`}
              ListEmptyComponent={() => searchText.trim().length > 0 ? (
                <TouchableOpacity style={[styles.categoryItem, { alignItems: "center" }]} onPress={() => { if (modalType === "crop") setCrop(searchText); else setWork(searchText); setModalType(null); setSearchText(""); setActiveInput(null); }}>
                  <AppText style={{ color: '#16A34A', fontWeight: '600' }}>{language === "te" ? `"${searchText}" ని చేర్చండి +` : `Add "${searchText}" +`}</AppText>
                </TouchableOpacity>
              ) : null}
              renderItem={({ item }) => (
                <TouchableOpacity style={styles.categoryItem} onPress={() => { const value = language === "te" ? item.te : item.en; modalType === "crop" ? setCrop(value) : setWork(value); setModalType(null); setSearchText(""); setActiveInput(null); }}>
...
const styles = StyleSheet.create({
  ...
  saveBtn: { marginBottom: 40, borderRadius: 18, overflow: "hidden", elevation: 6, shadowColor: "#1B5E20", shadowOffset: { width: 0, height: 6 }, shadowOpacity: 0.4, shadowRadius: 8},
